...
Col.Validator = function() {
  ...
  var MESSAGE = {
    ...
    URL       : "{0} 只能输入URL格式。",
    EMAIL     : "{0} 无效的电子邮件格式。",
    MINBYTES  : "{0} 必须输入至少{1}BYTE。",
    MAXBYTES  : "{0} 不能超过{1}BYTE。",
    MINLENGTH : "{0} 必须输入至少{1}个字符。",
    MAXLENGTH : "{0} 不能超过{1}个字符。",
    LENGTH    : "{0} 输入范围是{1}~{2}。",
    MIN       : "{0} 大于{1}。",
    MAX       : "{0} 小于{1}。",
    RANGE     : "{0) {1}~{2}以内",
    MINCHECK  : "{0} 至少确认{1}个以上 。",
    CHECK     : "{0} 请确认{1}~{2}。",
    PATTERN   : "{0} 无效的输入格式。",
    EQUALTO   : "{0} 与{1}相同。"    
  };
	...
	function setEventHandler() {
		...
		fileElement.find("input[name='fileData']").on('change', function(event){
			...
			if (upfile.size > settings.maxFileSize) {
				settings.validateFunc(1, "上传文件的最大范围是 " + Math.round(maxSize/1024/1024,1) + "MB");
				return;
			}
			...
			if (!matchedExt) {
				settings.validateFunc(1, "上传的扩展名是 " + availableExt.toString() + "");
				return;
			}
